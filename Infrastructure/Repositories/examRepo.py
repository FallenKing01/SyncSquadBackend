...
def get_approved_exams_for_student_group(studentId, limit):
    try:
        session = open_session()

        # Găsește grupa studentului
        student = session.query(Student).filter(Student.id == studentId).first()
        if not student:
            raise Exception("Student not found.")

        # Obține ID-ul grupei studentului
        student_group_id = student.idgrupa

        # Găsește examenele unde șeful de grupă (sefid) are aceeași grupă ca studentul
        exams = session.query(Examene).join(
            Student, Examene.sefid == Student.id
        ).filter(
            Student.idgrupa == student_group_id,  # Grupa șefului trebuie să coincidă
            Examene.starea == Status.APPROVED.name.lower(),  # Statusul APPROVED
            Examene.data > datetime.now().date()  # Data viitoare
        ).order_by(
            asc(Examene.data),
            asc(Examene.orastart)
        ).limit(limit).all()
        ...
        for exam in exams:
            ...
            data_serialized = exam.data.strftime("%Y-%m-%d") if isinstance(exam.data, (datetime, date)) else exam.data
            orastart_serialized = exam.orastart.strftime("%H:%M") if exam.orastart else None
            orafinal_serialized = exam.orafinal.strftime("%H:%M") if exam.orafinal else None
        ...
        raise Exception(f"Error while fetching exams for student {studentId}: {str(e)}")
    
def get_approved_exams_for_professor(professorId, limit):
    try:
        session = open_session()

        # Găsește examenele unde profesorul este cel specificat
        exams = session.query(Examene).filter(
            Examene.profesorid == professorId,  # Profesorul curent
            Examene.starea == Status.APPROVED.name.lower(),  # Statusul APPROVED
            Examene.data > datetime.now().date()  # Data viitoare
        ).order_by(
            asc(Examene.data),
        ...
        raise Exception(f"Error while fetching exams for professor {professorId}: {str(e)}")